...
const swName = `${injectManifest ? 'claims-sw.js' : 'prompt-sw.js'}`;
...
describe('react: test-build', () => {
  it(`service worker is generated: ${swName}`, () => {
    ...
    if (!injectManifest) {
      ...
      if (match) {
        ...
        expect(
          ...
          'missing workbox:core module. workboxName:' + workboxName,
        ).toBeTruthy();
        match = swContent.match(/self\.skipWaiting\(\);?\s*workbox\.clientsClaim\(\)/);
        expect(
          match && match.length === 1,
          'missing self.skipWaiting and clientsClaim workbox calls',
        ).toBeTruthy();
      } else {
        match = swContent.match(/workbox:core:[0-9.]+/);
        expect(match && match.length === 1, 'missing workbox:core module').toBeTruthy();
        match = swContent.match(/self\.skipWaiting\(\)/);
        expect(match && match.length === 1, 'missing self.skipWaiting workbox calls').toBeTruthy();
      }
    }
    ...
    expect(
      match && match.length === 1,
      'missing entry point route (index.html) in sw precache manifest',
    ).toBeTruthy();
    match = injectManifest
      ? swContent.match(/"url":\s*"claims-sw.js"/)
      : swContent.match(/"url":\s*"sw.js"/);
